...
toggle_paired() {
  if device_paired "$1"; then
    ...
    bluetoothctl pair "$1" >/dev/null 2>&1
    device_menu "$device"
  fi
}
...
toggle_trust() {
  if device_trusted "$1"; then
    bluetoothctl untrust "$1" >/dev/null 2>&1
    device_menu "$device"
  else
...
show_menu() {
  ...
  "$pairable")
    ...
    ;;
  *)
    device=$(bluetoothctl <<<"devices" | grep "^Device" | grep -F "$chosen")
...
rofi_command="rofi -dmenu -font JetBrainsMono\ Nerd\ Font\ 12 -p"
case "$1" in
--status)
  print_status
  ;;
